...
public class MessageElementTagProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext context, IProcessableElementTag tag, IElementTagStructureHandler structureHandler) {
        ...
        model.add(modelFactory.createStandaloneElementTag("th:block", "th:include", "fragments/MensagensSucesso"));//elemento, atributo, valor atributo
        model.add(modelFactory.createStandaloneElementTag("th:block", "th:include", "fragments/MensagensErroValidacao"));

        structureHandler.replaceWith(model, true); //true indica que é necessário o processamento das tags acima pelo thymeleaf, pois não são tags html.
    }
}
